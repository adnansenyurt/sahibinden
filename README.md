...
- Saved listing pages: open your saved listing (search results) HTML (e.g., file:///.../listing.html). The extension will scrape only the fields available in the listing rows (title, price, location, image, id, URL). It will NOT fetch or process detail pages in the background for file:// listings. The extension will still post each scraped row to the Emlak app with these minimal fields if you are logged in from the popup.
